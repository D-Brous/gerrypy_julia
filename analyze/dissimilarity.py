...
def convert_plans(plans, num_blocks):
    ...
    arr = np.array(converted_plans).astype(int)
    return arr
 
def vectorized_hamming_dist_matrix(plans):
    """
    Given a set of maps, return distance matrix
    Algorithm used: 
    similarity = sum_{i,j in nodes} s_ij,  
    s_ij = 1 if block i and j appear in the same district in both Plan 1 and Plan 2
    To convert to distance matrix: max - similarity
    Args:
      maps: (ndarray) a 3 dimensional tensor constructed using convert_plans
    """
    block_coocurrence_tensor = np.einsum('ijk, kli->ijl', plans, plans.T)
    output = np.einsum('ijk, kjl -> il', block_coocurrence_tensor, block_coocurrence_tensor.T)
