...
def make_bdm(leaf_nodes, n_blocks=None):
    ...
    block_district_matrix = np.zeros((n_blocks, len(districts)), dtype = int)
...
def generation_metrics(cg, low_memory=False):
    ...
    print(block_district_matrix)
    print(type(block_district_matrix[0][0]))
...
def dispersion_compactness(districts, state_df):
    ...
    compactness_scores = []
    for d in districts:
        population = state_df.loc[d]['TOTAL_ADJ'].values
...
def vectorized_dispersion(bdm, state_df, bdm_shard_size=100_000):
    ...
    population = state_df.TOTAL_ADJ.values
...
def aggregate_average_metrics(bdm, metric_df, weights):
    ...
    print(type(district_weight_normalizer[0]))
    print(type((bdm.T * weights)[0][0]))
    print(type((metric_df.values)[0][0]))
    #print(((bdm.T * weights) @ metric_df.values).T)
    print((bdm.T * weights))
    print(metric_df.values)
    print((bdm.T * weights) @ metric_df.values)
    #print((((bdm.T * weights) @ metric_df.values).T / district_weight_normalizer).T)
    return pd.DataFrame(
...
def create_district_df(state, bdm, special_input=''):
    ...
    state_df, G, _, _ = load_opt_data(special_input)

    sum_metrics = state_df[['area', 'TOTAL_ADJ']]
    average_metrics = state_df[[]]
    average_metrics = state_df.drop(columns=['area', 'TOTAL_ADJ', 'GEOID'])

    sum_metric_df = aggregate_sum_metrics(bdm, sum_metrics)
    pop_weights = [int(v) for v in state_df.TOTAL_ADJ.values]
...
def create_objective_df(state, leaf_nodes, internal_nodes, fixed_std=None):
    ...
    entropy = boundary_entropy(bdm, bbm, state_df.TOTAL_ADJ.values)
