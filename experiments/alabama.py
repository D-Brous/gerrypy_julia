...
class Experiment:
    ...
    def run(self):
        ...
        state_df, G, lengths, edge_dists = load_opt_data(state='AL')

        maj_min=majority_minority(bdm, state_df)
        bb=black_belt(bdm, state_df)
        print(maj_min)
        print(sum(maj_min))
        print(bb)
        ...
        solutions, sol_tree = master_solutions(bdm,cg.leaf_nodes, cg.internal_nodes, state_df, lengths,G, maj_min,bb) 
        print(solutions)
...
def master_solutions(bdm,leaf_nodes, internal_nodes, state_df, lengths, G, maj_min, bb):
    """
...
if __name__ == '__main__':
    ...
    pdp_config = {
        'state': 'AL',
        'n_districts': 7,
        'population_tolerance': .015, #the max value in the current plan is 0.01906
        'required_mm': 0, #TODO if this is 0, partition stage works
        #'population_tolerance': get_population_tolerance('AL'),
    }
...
#TODO
#Add column to state_df w 1 if block is in black belt, 0 else
#before solving master, dot product of list of BB 0-1s with the corresponding bdm column for each district
#if dot product > 0, dist is in BB
#similar to MM, in master problem, no more than 2 BB dists
